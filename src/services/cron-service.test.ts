...
describe('CronService', () => {
  ...
  describe('getCronRequestById', () => {
    ...
    it('should return cron request by id', async () => {
      ...
      expect(retrieved?.name).toBe('req1')
    })

    it('should return null for non-existent request', async () => {
      const retrieved = await service.getCronRequestById(999)
  ...
  describe('approveCronRequest', () => {
    ...
    it('should throw error when request not found', async () => {
      await expect(service.approveCronRequest(999, 'reviewer1')).rejects.toThrow('Cron request 999 not found')
    })
  ...
  describe('rejectCronRequest', () => {
    ...
    it('should reject a pending cron request', async () => {
      ...
      expect(rejected.reviewed_by).toBe('reviewer1')
      expect(rejected.reviewer_notes).toBe('Invalid schedule')
    })

    it('should throw error when request not found', async () => {
      await expect(service.rejectCronRequest(999, 'reviewer1')).rejects.toThrow('Cron request 999 not found')
    })
