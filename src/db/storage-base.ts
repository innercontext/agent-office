...
export abstract class AgentOfficeStorageBase implements AgentOfficeStorage {
  ...
  abstract listMessagesForRecipient(name: string, filters?: {unread?: boolean, olderThanHours?: number, notified?: boolean}): Promise<MessageRow[]>
  ...
  abstract createCronJob(name: string, sessionName: string, schedule: string, timezone: string | null, message: string): Promise<CronJobRow>
