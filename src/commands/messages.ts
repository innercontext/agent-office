...
export async function checkUnreadMail(storage: AgentOfficeStorage, coworkerName: string, useJson: boolean): Promise<void> {
  const service = new MessageService(storage)
  const hasUnread = await service.checkUnreadMail(coworkerName)
  console.log(formatOutput({ hasUnread }, useJson))
}

export async function getUnreadMail(storage: AgentOfficeStorage, coworkerName: string, useJson: boolean): Promise<void> {
  const service = new MessageService(storage)
