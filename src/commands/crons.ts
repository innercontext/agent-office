...
export async function cronHistory(storage: AgentOfficeStorage, id: number, limit: number, useJson: boolean): Promise<void> {
  const service = new CronService(storage)
...
export async function requestCron(
  ...
  const service = new CronService(storage)
  const request = await service.createCronRequest(name, sessionName, schedule, timezone ?? null, message)
...
export async function createCron(
  ...
  const service = new CronService(storage)
  const job = await service.createCronJob(name, sessionName, schedule, timezone ?? null, message)
  console.log(formatOutput(job, useJson))
}
